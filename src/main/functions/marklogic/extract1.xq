...
{
	typeswitch ($node)
		case $a as element(apidoc:example) return <example xml:space="preserve">{if ($node/xhtml:pre) then $node/xhtml:pre/node() else $node/node()}</example>
		case element() return element { $node/local-name(.) } { $node/@*, local:remove-namespaces ($node/node()) }
		default return $node
};

let $version := xdmp:get-request-field('version', "6.0")
let $base-uri := fn:concat ('http://docs.marklogic.com/', $version)
let $get-options :=
